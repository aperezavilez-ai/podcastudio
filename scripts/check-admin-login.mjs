const email = process.env.ADMIN_EMAIL || 'alfonsoavilez@icloud.com'
...
async function checkProduction() {
  try {
    const html = await fetch('https://www.podcastudio.mx/').then((r) => r.text())
    const jsPath = html.match(/src="(\/assets\/index-[^"]+\.js)"/)?.[1]
    if (!jsPath) {
      console.log('prod: no JS bundle found')
      return
    }
    const code = await fetch(`https://www.podcastudio.mx${jsPath}`).then((r) => r.text())
